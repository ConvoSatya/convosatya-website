  ...
  try {
    if (!BACKEND_URL || !FRONTEND_SHARED_SECRET) {
      return NextResponse.json(
        { ok: false, message: "FAUST backend is not configured." },
        { status: 500 }
      );
    }
    ...
    const backendResponse = await fetch(`${BACKEND_URL}/drive/media/restore`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      {
        ok: false,
        message:
          error instanceof Error ? error.message : "Drive media restore failed.",
      },
      { status: 500 }
    );
  }
